...
size_filter = os.environ.get("SSL_NGINX_SIZES")
if size_filter:
    all_sizes = SIZES
    requested_sizes = [item.strip() for item in size_filter.split(",") if item.strip()]
...
def run_benchmark(size_name, size_bytes):
    ...
    inner_config = {}
    inner_results = {}
    result_match = re.search(r"Results saved to\s+(\S+benchmark_results_\d+_\d+\.json)", output)
    if result_match:
        inner_result_path = result_match.group(1)
        try:
            with open(inner_result_path) as f:
                inner_data = json.load(f)
            inner_stats = inner_data.get("stats", {})
    ...
    for test_type in ["baseline", "kernel_sslsniff", "bpftime_sslsniff"]:
        values = inner_results.get(test_type, [])
        if values:
            results[test_type] = sum(values) / len(values)
            continue

        match = re.search(
            rf"{test_type.replace('_', ' ').title()}:.*?Requests/sec \(mean\):\s+(\d+\.\d+)",
            output,
...
def plot_results(all_results, output_dir):
    ...
    # Save absolute performance plot. Previous sudo runs may leave a root-owned
    # absolute_performance.png, so fall back to a timestamped filename.
    abs_plot_path = f"{output_dir}/absolute_performance.png"
    try:
        plt.savefig(abs_plot_path)
    except PermissionError:
        fallback_path = f"{output_dir}/absolute_performance_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png"
        print(f"Cannot write {abs_plot_path}; saving plot to {fallback_path} instead")
